...
dm$FAM<-paste(dm$SuperOrder, dm$Order, dm$Family)
dn.fam<-sapply(split(dm$dn, dm$FAM),sum, na.rm=TRUE)
ds.fam<-sapply(split(dm$ds, dm$FAM),sum, na.rm=TRUE)
dnds.fam<-sapply(split(dm$dn/dm$ds, dm$FAM),median, na.rm=TRUE)
mass.fam<-sapply(split(dm$mass, dm$FAM),median, na.rm=TRUE)
brle.fam<-sapply(split(dm$Branch.length, dm$FAM),sum, na.rm=TRUE)
...
dnds.ord<-sapply(split(dm$dn/dm$ds, dm$Order),median, na.rm=TRUE)
mass.ord<-sapply(split(dm$mass, dm$Order),median, na.rm=TRUE)
