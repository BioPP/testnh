# Created on 28/05/2011 by jdutheil

########################################
# Profiling:
free.aic<-read.table("lysozymeSmall.model_free_AIC.log.txt", header=TRUE, sep="\t")
join.aic<-read.table("lysozymeSmall.model_join_AIC.log.txt", header=TRUE, sep="\t")
...
free.aic.prf<-read.profile("lysozymeSmall.free_AIC.profile")
join.aic.prf<-read.profile("lysozymeSmall.join_AIC.profile")

#Read partition tables:
brlen<-read.table("lysozymeSmall.ml_h.brlen.txt", sep="\t", header=TRUE, row.names="Id")

free.aic.tbl<-read.table("lysozymeSmall.partitions_record_free_AIC.txt", sep="\t")
join.aic.tbl<-read.table("lysozymeSmall.partitions_record_join_AIC.txt", sep="\t")
...
plot.profile<-function(prf, iterations, omegas, brlen, tbl, ics) {
  ...
  plot.new()
  plot.window(xlim=range(prf$Time), ylim=c(0,5.0))
  axis(2)
  ...
  for (it in iterations) {
    ...
    rect(time[1], -0.5, time[length(time)], 5.5, col=bgcol[it], border=NA)
    for (omega in omegas) {
...
plot.profile(free.aic.prf, 1:9, 1:9, brlen, free.aic.tbl, free.aic)
dev.print(tiff, file="Figures/FreeLysozymeSmall.tif", width=1000, height=500)

plot.profile(join.aic.prf, 1:7, 1:9, brlen, join.aic.tbl, join.aic)
dev.print(tiff, file="Figures/JoinLysozymeSmall.tif", width=1000, height=500)
